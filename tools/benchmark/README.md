...
1. 确保 zookeeper 已经启动 (dubbo和triple的测试需要依赖zookeeper)
...
```
go build -o dist/pixiu cmd/pixiu/pixiu.go
mv dist/ tools/benchmark/
```
...
```
go test -v test/dubbo_suite/dubbo_test.go 
```

grpc:

```
go test -v test/grpc_suite/grpc_test.go 
```

triple:

```
go test -v test/triple_suite/proto_suite/proto_test.go 
```
